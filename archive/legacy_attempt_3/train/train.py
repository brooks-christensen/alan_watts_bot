...
import os
from torch import load as torchload, device as torchDevice, Tensor
...
def train_batch(
    ...
    return total_train_loss, t0


def validation_batch(model: GPT2LMHeadModel, validation_dataloader: DataLoader, device: torchDevice):
...
def train(
    ...
    for epoch_i in range(0, config['training']['epochs']):
        ...
        total_train_loss, t0 = train_batch(
            epoch_i=epoch_i, 
            config=config, 
            model=model, 
        ...
        avg_val_loss, t0 = validation_batch(model=model, validation_dataloader=validation_dataloader, device=device)
...
def resume_training_if_possible(
        model: GPT2LMHeadModel, 
        optimizer: AdamW, 
        scheduler: LambdaLR, 
        config: dict,
    ...
    config (dict): the configuration dictionary containing paths and other settings

    Returns:
    --------
    int: the starting epoch for resuming training
    """
